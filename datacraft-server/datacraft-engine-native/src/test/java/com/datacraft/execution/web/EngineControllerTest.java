...
class EngineControllerTest {
    ...
    @Test
    @WithMockUser
    void returnsSingleEngineByCode() throws Exception {
        mockMvc.perform(get("/api/v1/engines/NATIVE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.code").value("NATIVE"))
                .andExpect(jsonPath("$.data.health.status").value("UP"));
    }

    @Test
    @WithMockUser
    void testsEngineHealthByCode() throws Exception {
        mockMvc.perform(post("/api/v1/engines/NATIVE/test"))
                .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser
    void updatesEngineConfiguration() throws Exception {
        mockMvc.perform(put("/api/v1/engines/NATIVE"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.code").value("NATIVE"));
    }
    ...
    @Test
    @WithMockUser
    void returnsNotFoundForUnknownEngine() throws Exception {
        mockMvc.perform(get("/api/v1/engines/UNKNOWN"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("ENGINE_NOT_FOUND"));
    }
