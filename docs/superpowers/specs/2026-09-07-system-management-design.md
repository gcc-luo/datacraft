...
补齐一个受管理员角色保护的系统管理模块，采用已确认的“系统设置二级菜单”布局：主侧边栏显示“系统设置”一级菜单，展开后显示“用户管理、角色管理、菜单管理”三个二级入口，右侧显示对应列表和编辑区域。
...
- 从现有动态导航进入“系统设置”，并通过二级菜单进入 `/system/users`、`/system/roles`、`/system/menus`。
- 访问旧地址 `/system` 时自动跳转到 `/system/users`，保证历史链接继续可用。
...
新增 `SystemManagementView.vue` 和系统管理嵌套路由。页面使用与现有控制台一致的深蓝侧栏、白色内容卡片和蓝色主操作色；系统管理资源入口由主侧边栏的动态菜单树承载，内容区域不再重复显示资源导航：

- 用户管理：用户列表、创建/编辑抽屉、角色多选、启用状态。
- 角色管理：角色列表、创建/编辑抽屉、菜单树勾选授权、启用状态。
- 菜单管理：菜单列表、创建/编辑抽屉、父级菜单选择、路由与排序编辑。

系统设置一级菜单及三个二级菜单由 Flyway 初始化并授予 ADMIN 角色，普通用户只看到其被授权的菜单。接口返回 403 时显示权限不足提示，不回退到空白页面。创建/编辑成功后刷新当前列表，并显示成功提示；加载和提交过程显示禁用状态；表单错误显示后端返回消息。
